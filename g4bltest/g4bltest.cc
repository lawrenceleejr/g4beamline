 ...
 *		if a named test file does not exist, it is run from the test
 ...
 *		FAILURE prints the test name and title, plus the error message
...
// This program uses only sstdlib includes, in particular no Qt. But it was
...
int main(int argc, char *argv[])
{
	...
	// get binDir
	binDir = g4blDir + "/bin";
	if(!fileExists((binDir+"/g4bl" EXE).c_str())) {
	    binDir = g4blDir + "/MacOS";
	    if(!fileExists((binDir+"/g4bl" EXE).c_str())) {
		fprintf(stderr,"g4bltest: cannot find G4beamline programs\n");
		exit(2);
	    }
	}
	...
	// if file not found in ., use it from g4blDir/tests
	for(int i=0; i<tests.size(); ++i) {
		FILE *in = fopen(tests[i].c_str(),"r");
		if(!in) {
			tests[i] = g4blDir + "/test/" + tests[i];
		} else {
			fclose(in);
		}
	}
	...
	do {
		...
		// print summary
		if(nFail == 0) {
	   		printf("All tests passed (%d loops, %d total, "
					"%d omitted)\n",
	    				nLoop,nSuccess+nOmit,nOmit);
		} else {
	   		printf("***g4bltest: %d loops, %d tests failed out of "
				"%d tests (%d omitted)\n",
				nLoop,nFail,nFail+nSuccess+nOmit,nOmit);
	   		if(!loop) {
...
TestResult runTest(String filename)
{
	...
	FILE *copyin = fopen(filename.c_str(),"r");
	FILE *copyout = fopen(test.c_str(),"w");
	if(copyin != 0 && copyout != 0) {
		for(;;) {
			char buf[4096];
			char *p = fgets(buf,sizeof(buf),copyin);
			if(!p) break;
			fputs(buf,copyout);
		}
	}
