...
#ifdef G4BL_VISUAL
	viewMenu->addAction("Viewer = OpenGLStoredQt");
	viewMenu->addAction("Viewer = Other Viewer");
#endif
	viewMenu->addAction("No Viewer");
	// Qt adds enter/exit full screen to the viewMenu
	connect(viewMenu,SIGNAL(triggered(QAction*)),
						this,SLOT(setVisual(QAction*)));
	toolMenu->addAction("Do Regression Tests",
						this,SLOT(doRegressionTests()));
	toolMenu->addAction("Download Data sets",
						this,SLOT(downloadDataSets()));
	helpMenu->addAction("Help");
	helpMenu->addAction("Users Guide");
	helpMenu->addAction("Validation Document");
	helpMenu->addAction("README");
	connect(helpMenu,SIGNAL(triggered(QAction*)),
						this,SLOT(setHelp(QAction*)));
...
void G4blGuiWindow::setVisual(QAction *action)
{
#ifdef G4BL_VISUAL
	if(action != 0) {
	    QString s = action->text();
	    if(s.contains("No Viewer")) {
		visual->setChecked(false);
		visClicked();
	    } else if(s.contains("OpenGL") || s.contains("OGL")) {
		visual->setChecked(true);
		visClicked();
	    } else {
		visual->setChecked(false);
...
void G4blGuiWindow::setHelp(QAction *action)
{
	QUrl url;
	url.setScheme("file");
	if(action != 0) {
...
void G4blGuiWindow::downloadDataSets()
{
	QString path = g4bl_dir + "/" BIN "/g4bldata";
	system(qPrintable(path + "&"));
}
